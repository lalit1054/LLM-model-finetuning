...
class EvalConfig:
    ...
    finetuned_model_path: Path = PROJECT_ROOT / "models" / "qwen3.5-0.8b-chatdoctor-unsloth/checkpoint-700"
    ...
    max_new_tokens: int = env_int("EVAL_MAX_NEW_TOKENS", 512)
...
CONFIG.finetuned_model_path = resolve_existing_path(
    ...
    PROJECT_ROOT / "models" / "qwen3.5-0.8b-chatdoctor-unsloth/checkpoint-700",
)
...
def load_base_model_and_tokenizer(config: EvalConfig):
    model, tokenizer, device = load_model(config.base_model_path)
    return model, tokenizer


def load_finetuned_model_and_tokenizer(config: EvalConfig):
    model, tokenizer, device = load_model(config.finetuned_model_path)
...
def run_inference_pass(
    ...
    for index, row in enumerate(rows, start=1):
        ...
        print(f'time taken in model prediction is {round(time.time()-t1,2)}')
    return predictions
